...
module.exports = async ({
  ...
  try { 
    ...
    if (!awsApi) {
      ...
      const localSwaggerExt = apiMerger.mergeMockExtensions(localSwagger, basePath, mediaTypes);

      // create the API by importing a new Swagger
      //importedApi = await apiGtw.createApiFromSwagger(JSON.stringify(localSwaggerExt));
      importedApi = JSON.stringify(localSwaggerExt);
      ...
      const mergedSwaggerExt = apiMerger.mergeMockExtensions(mergedSwagger, basePath, mediaTypes);
      ...
      importedApi = JSON.stringify(mergedSwaggerExt);    
    }
