...
module.exports = async ({
  ...
  try { 
    ...
    if (!awsApi) {
      ...
      const localSwaggerExt = apiMerger.mergeMockExtensions(localSwagger);
      ...
      const mergedSwaggerExt = apiMerger.mergeMockExtensions(mergedSwagger);

      // update existing API by importing the new Swagger
      importedApi = await apiGtw.importApi(awsApi.id, JSON.stringify(mergedSwagger));      
    }
